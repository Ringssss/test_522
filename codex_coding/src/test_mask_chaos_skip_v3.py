#!/usr/bin/env python3
"""
Token Skip v3 — Fixed inplace bug.

Previous experiments had a bug: fused_experts uses inplace=True, so hs_flat
gets overwritten with routed output. The "identity" was actually
routed_output * weight_sum, not original_hs * weight_sum.

Fix: clone hs_flat before forward_impl to preserve original values.
...
def main():
    ...
    print("=" * 80)
    print("Token Skip v3 — Fixed inplace bug (true identity)")
    ...
    with set_current_vllm_config(VllmConfig(parallel_config=pcfg)):
        ...
        def patch_skip_fixed(ctrl):
            ...
            for li, layer in enumerate(model.model.layers):
                ...
                # Patch MoE forward: shared + routed (with FIXED identity for skip tokens)
                def mk_moe_fwd(blk, layer_i, rsf_val):
                    def fwd(hidden_states):
                        ...
                        # ★ FIX: save original hs BEFORE forward_impl (which is inplace)
                        th = state['chaos_threshold']
                        prev_conf = state['prev_confidence']
                        need_skip = (th > 0 and prev_conf is not None and prev_conf.shape[0] == N)

                        if need_skip:
                            hs_original = hs_flat.clone()  # ← preserve before inplace

                        logits = blk.gate.get_logits(hs_flat)
                        routed = blk.experts.forward_impl(
                            hidden_states=hs_flat, router_logits=logits)
                        # After forward_impl: hs_flat == routed (inplace)
                        ...
                        if need_skip:
                            ...
                            if n_skip > 0:
                                ...
                                if topk_w is not None:
                                    ...
                                    # ★ Use ORIGINAL hs for identity, not the inplace-modified one
                                    weight_sum = topk_w.sum(dim=1, keepdim=True).to(hs_original.dtype)
                                    identity = hs_original * weight_sum
                                    routed = torch.where(
                ...
                moe.forward = mk_moe_fwd(moe, eb_li, r)
        ...
        def reset_state(ctrl):
            ...
            state['skip_counts'] = []; state['total_counts'] = []
        ...
        for th in THRESHOLDS:
            ...
            results[label] = {
                ...
                'skip_rate': skip_rate, 'fwd_t7': fwd_t7,
            }
        ...
        # Summary
        print(f"\n{'='*80}")
        print("SUMMARY — True Identity Skip (inplace bug fixed)")
        print(f"{'='*80}")
        ...
        for label, r in results.items():
            print(f"  {label:<12s} {r['time_s']:>8.3f} {r['fwd_t0']:>8d} {r['ms_per_fwd']:>8.2f} "
                  f"{r['skip_rate']:>6.1%} {r['fwd_t7']:>8d}")

        out_path = REPO_ROOT / "codex_coding" / "results" / "mask_chaos_skip_v3_fixed.json"
