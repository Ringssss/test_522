#!/usr/bin/env python3
"""
Benchmark BD-attnmask + IterSmooth combination vs BD-attnmask baseline.

Tests:
  A. BD attnmask (baseline, no cache)
  B. BD attnmask + IterSmooth (default params)
  C. BD attnmask + IterSmooth (higher cont_weight=0.5)
  D. BD attnmask + IterSmooth (no threshold_decay)
...
def run_suite(label, input_ids, device, model, make_decoder):
    ...
    from dinfer import (
        ...
        KVCacheFactory,
        IterSmoothBlockDiffusionLLMAttnmask,
        IterSmoothBlockDiffusionLLMCache,
    )
    ...
    # B. BD + prefix cache (reference, no IterSmooth)
    ...
    results.append(bench_dllm("B: BD + prefix cache", dllm, input_ids, device))

    # C. BD+IterSmooth no-cache (high_w, best from last round)
    dllm = IterSmoothBlockDiffusionLLMAttnmask(
        model, make_decoder(),
        BlockIteratorFactory(use_block_diffusion=True),
        early_stop=True,
        cont_weight=0.5, cont_weight_init=0.25,
        cont_weight_growth=0.03, threshold_decay=0.02,
    )
    results.append(bench_dllm("C: BD+IS no-cache high_w", dllm, input_ids, device))

    # D. BD+IterSmooth + prefix cache (default params)
    ...
    results.append(bench_dllm("D: BD+IS+cache default", dllm, input_ids, device))

    # E. BD+IterSmooth + prefix cache (high_w)
    dllm = IterSmoothBlockDiffusionLLMCache(
        model, make_decoder(),
        BlockIteratorFactory(use_block_diffusion=True),
        cache_factory=KVCacheFactory("prefix", is_bd_model=True),
    ...
    results.append(bench_dllm("E: BD+IS+cache high_w", dllm, input_ids, device))
...
def print_summary(label, results, prompt_len):
    ...
    print(f"{'='*95}")
    print(f"{'Path':<35} {'tok/s':<10} {'fwd/s':<10} {'tokens':<8} {'fwd':<8} {'time(s)':<10}")
    print("-" * 95)
...
def main() -> None:
    ...
    # Save results
    output_path = RESULTS_DIR / "bd_itersmooth_benchmark_results.json"
