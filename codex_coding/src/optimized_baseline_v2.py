#!/usr/bin/env python3
"""
HetEval-32: Optimized Baseline v2 — max_unroll=4 + Fused RMSNorm + Flash-Attn-4

Three baseline optimizations:
  1. maximum_unroll=4
  2. Fused RMSNorm (vllm kernel)
  3. Flash-Attn-4 SM90 kernel (replaces PyTorch SDPA)

Wall-clock timing + output quality verification on 5 verifiable prompts.
"""
...
def apply_flash_attn_4(model):
    """Replace SDPA attention with flash-attn-4 SM90 kernel."""
    from flash_attn.cute import flash_attn_func
    from dinfer.model.modeling_llada2_moe import (
        LLaDA2MoeSdpaAttention, apply_rotary_pos_emb, repeat_kv
    )
    ...
    for name, module in model.named_modules():
            ...
            def fa4_forward(
                ...
                **kwargs,
                ...
                if output_attentions:
                    # Fall back to parent for output_attentions
                    return LLaDA2MoeSdpaAttention.forward(
                        attn_mod, hidden_states, attention_mask, position_ids,
                        past_key_value, output_attentions, use_cache,
                        position_embeddings, cache_position, replace_position, **kwargs)
                ...
                qkv = qkv.view(bsz, q_len,
                               attn_mod.num_heads // attn_mod.tp_size +
                               2 * attn_mod.num_key_value_heads // attn_mod.tp_size,
                               attn_mod.head_dim)
                query_states, key_states, value_states = qkv.split(
                    [attn_mod.num_heads // attn_mod.tp_size,
                     attn_mod.num_key_value_heads // attn_mod.tp_size,
                     attn_mod.num_key_value_heads // attn_mod.tp_size], dim=-2)

                # (B, S, H, D) -> (B, H, S, D) for QK norm and RoPE
                query_states = query_states.transpose(1, 2)
                key_states = key_states.transpose(1, 2)
                value_states = value_states.transpose(1, 2)

                # QK norm + RoPE (same as original)
                query_states = attn_mod.query_layernorm(query_states)
                key_states = attn_mod.key_layernorm(key_states)

                cos, sin = position_embeddings
                query_states, key_states = apply_rotary_pos_emb(
                    query_states, key_states, cos, sin)

                # Cache update (same as original)
                ...
                # --- Flash-Attn-4 path ---
                # Convert (B, H, S, D) -> (B, S, H, D) for flash_attn_func
                ...
                if attention_mask is not None:
                    ...
                    query_states_c = query_states.contiguous()
                    key_states_c = key_states_expanded.contiguous()
                    value_states_c = value_states_expanded.contiguous()
                    am = attention_mask.bool() if attention_mask is not None else None
                    if am is not None and am.dim() == 3:
                        am = am.unsqueeze(1)
                    attn_output = F.scaled_dot_product_attention(
                        query_states_c, key_states_c, value_states_c,
                        attn_mask=am, dropout_p=0.0, is_causal=False)
                else:
                    # Flash-Attn-4 with native GQA support
                    attn_output_fa, _ = flash_attn_func(
                        q_fa, k_fa, v_fa, causal=False)
                    # (B, S, H, D) -> (B, H, S, D)
                    attn_output = attn_output_fa.transpose(1, 2)
        ...
        attn.forward = make_fa4_forward(attn)
        count += 1
...
def main():
    ...
    print("=" * 80)
    print("HetEval-32: Optimized Baseline v2")
    print("  max_unroll=4 + fused RMSNorm + flash-attn-4 SM90")
    print("=" * 80)
    ...
    with set_current_vllm_config(VllmConfig(parallel_config=pcfg)):
        ...
        # ---- Config A: prev optimized baseline (unroll=4, fused RMSNorm, SDPA) ----
        ...
        fwd_a = dllm_a.diff_iteration.num_forwards
        time_a = t1 - t0
        print(f"  Fwd={fwd_a}, time={time_a:.3f}s, fwd/s={fwd_a/time_a:.1f}")

        # ---- Config B: full optimized baseline (+ flash-attn-4) ----
        print("\n--- Config B: unroll=4 + fused RMSNorm + flash-attn-4 ---")
        n_fa4 = apply_flash_attn_4(model)
        print(f"  Flash-Attn-4: {n_fa4} attention modules")
        ...
        print(f"SUMMARY")
        print(f"{'='*80}")
        print(f"  {'Config':<45s} {'Fwd':>4s} {'Time(s)':>8s} {'Fwd/s':>6s}")
        print(f"  {'-'*65}")
        print(f"  {'A: unroll=4 + fused RMSNorm (SDPA)':<45s} {fwd_a:>4d} {time_a:>8.3f} {fwd_a/time_a:>6.1f}")
        print(f"  {'B: A + flash-attn-4 SM90':<45s} {fwd_b:>4d} {time_b:>8.3f} {fwd_b/time_b:>6.1f}")
        if time_b < time_a:
            pct = (time_a - time_b) / time_a * 100
            print(f"\n  Flash-Attn-4 additional speedup: {pct:.1f}%")
        else:
            pct = (time_b - time_a) / time_a * 100
            print(f"\n  Flash-Attn-4 overhead: +{pct:.1f}%")

        save_data = {
            "config_a": {"fwd": fwd_a, "time_s": time_a, "desc": "unroll=4 + fused RMSNorm + SDPA"},
            "config_b": {"fwd": fwd_b, "time_s": time_b, "desc": "unroll=4 + fused RMSNorm + flash-attn-4"},
        }
        out_path = REPO_ROOT / "codex_coding" / "results" / "optimized_baseline_v2_timing.json"
