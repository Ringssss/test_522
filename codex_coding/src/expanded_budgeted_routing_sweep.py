#!/usr/bin/env python3
"""
v0.1.15.5 — Expanded Budgeted Routing: per_round_cap & quality_floor Sweep

Sweep A: per_round_cap in {1,2,4,8,16,32,64}, QF=0.85
Sweep B: QF in {0.40,0.50,0.60,0.70,0.80,0.85}, per_round_cap=8

All configs: M=4, K_target=40, max_rounds=999 (unlimited), no top-p.
...
class ExpandedBudgetedRoutingController:
    """Expanded top-(K+M) budgeted routing with per-layer stats."""
    ...
    def __init__(self, K=8, M=4, K_target=40, quality_floor=0.85,
        ...
        # Per-layer stats: layer_idx -> list of values
...
def run_config(model, make_dllm, input_ids, prompt_len, tokenizer,
    ...
    """Run one config: timing + per-layer stats + quality."""
    GEN_LENGTH = 256
    ...
    if ctrl is not None:
        for li in sorted(ctrl.layer_active.keys()):
            ...
            per_layer_sat[li] = sum(ctrl.layer_sat[li]) / len(ctrl.layer_sat[li]) if ctrl.layer_sat[li] else 0
...
def main():
    ...
    print("=" * 80)
    print("Expanded Budgeted Routing — per_round_cap & quality_floor Sweep")
    print("  M=4, K_target=40, max_rounds=unlimited, no top-p")
    ...
    with set_current_vllm_config(VllmConfig(parallel_config=pcfg)):
        ...
        # ============================================================
        # Sweep A: per_round_cap (QF=0.85 fixed)
        # ============================================================
        print(f"\n{'='*80}")
        print("SWEEP A: per_round_cap (QF=0.85 fixed)")
        print(f"{'='*80}")

        for cap in [1, 2, 4, 8, 16, 32, 64]:
            cname = f"A_cap{cap}"
            print(f"\n--- {cname}: per_round_cap={cap}, QF=0.85 ---")
            ctrl = ExpandedBudgetedRoutingController(
                K=8, M=4, K_target=40, quality_floor=0.85,
                q_major=0.95, per_round_cap=cap, max_add_rounds=999)
            r = run_config(model, make_dllm, input_ids, prompt_len, tokenizer,
                           decoder_t0, decoder_t7, ctrl, cname)
            all_results[cname] = r
            delta = r["fwd"] - bl_fwd
        ...
        # ============================================================
        # Sweep B: quality_floor (per_round_cap=8 fixed)
        # ============================================================
        print(f"\n{'='*80}")
        print("SWEEP B: quality_floor (per_round_cap=8 fixed)")
        print(f"{'='*80}")

        for qf in [0.40, 0.50, 0.60, 0.70, 0.80, 0.85]:
            cname = f"B_qf{int(qf*100)}"
            print(f"\n--- {cname}: QF={qf}, per_round_cap=8 ---")
            ctrl = ExpandedBudgetedRoutingController(
                K=8, M=4, K_target=40, quality_floor=qf,
                q_major=0.95, per_round_cap=8, max_add_rounds=999)
            r = run_config(model, make_dllm, input_ids, prompt_len, tokenizer,
                           decoder_t0, decoder_t7, ctrl, cname)
            all_results[cname] = r
        ...
        print(f"\n  Sweep A (per_round_cap, QF=0.85):")
        print(f"  {'Config':<12s} {'Fwd':>4s} {'dFwd':>5s} {'|S|':>6s} "
        ...
        for qf in [0.40, 0.50, 0.60, 0.70, 0.80, 0.85]:
            ...
            ls = r["per_layer_S"]
            vals = [ls[str(i)] for i in range(19) if str(i) in ls] if ls else [0]
            print(f"  QF={qf:<5.2f}   {r['fwd']:>4d} {delta:>+4d} "
                  f"{r['global_active']:>6.1f} {r['global_sat']:>5.1%} "
                  f"{r['global_rounds']:>5.1f} {min(vals):>5.0f} {max(vals):>5.0f}")

        # Save
        out_path = (REPO_ROOT / "codex_coding" / "results" /
                    "expanded_budgeted_routing_sweep.json")
