#!/usr/bin/env python3
"""
v0.1.15.8c — C10-v3: K_init fix only (keep K_B_v2 Triton)

Isolate the K_init fix: use actual |S| from cold path, but keep
K_B_v2 Triton kernel (not torch.topk) in hot path.

C10-v1: K_init=168, K_B_v2        → baseline EB
C10-v3: K_init=actual, K_B_v2     → K_init fix only
C10-v2: K_init=actual, torch.topk → K_init fix + topk replacement
"""
...
# ================================================================
# V3: K_init fix + keep K_B_v2 Triton
# ================================================================
class FusedEBControllerV3(FusedEBController):
    """K_init from actual cold path |S|, but keep K_B_v2 Triton in hot path."""
    ...
    def cold_path(self, layer_idx, logits, bias):
        ...
        # ★ Count actual |S| + 2*cap margin (1 sync, cold only)
        actual_s = int(b['s_mask'].sum().item())
        k_init_with_margin = min(actual_s + 2 * self.cap, 256)
        self.K_init[layer_idx] = k_init_with_margin
        self.k_init_history.append((actual_s, k_init_with_margin))

        self.cold_count += 1
        return b['s_mask']

    # hot_path inherits from FusedEBController:
    #   K_A + K_B_v2 with K_init from self.K_init[layer_idx]
    #   (now set to actual |S| instead of 168)
...
def main():
    ...
    print("=" * 80)
    print("v0.1.15.8c — C10-v3: K_init = actual |S| + 2*cap, K_B_v2 Triton")
    ...
    with set_current_vllm_config(VllmConfig(parallel_config=pcfg)):
        ...
        def run_config(name, eb_ctrl=None):
            ...
            if eb_ctrl:
                ...
                if hasattr(eb_ctrl, 'k_init_history') and eb_ctrl.k_init_history:
                    actuals = [x[0] if isinstance(x, tuple) else x for x in eb_ctrl.k_init_history]
                    k_inits = [x[1] if isinstance(x, tuple) else x for x in eb_ctrl.k_init_history]
                    result['actual_s_avg'] = sum(actuals) / len(actuals)
                    result['k_init_avg'] = sum(k_inits) / len(k_inits)
                    result['k_init_min'] = min(k_inits)
                    result['k_init_max'] = max(k_inits)
            return result
        ...
        # C10-v1
        print(f"\n{'='*60}")
        print("C10-v1: K_init=168, K_B_v2")
        print(f"{'='*60}")
        ...
        # C10-v3 (★ new: K_init fix + K_B_v2)
        print(f"\n{'='*60}")
        print("C10-v3: K_init=actual |S|, K_B_v2 (Triton)")
        print(f"{'='*60}")
        ...
        print(f"  {'Config':<35s} {'Time(s)':>8s} {'Fwd':>5s} {'ms/fwd':>8s} "
              f"{'vs C5':>8s} {'|S|avg':>8s} {'K_init':>8s}")
        print(f"  {'-'*80}")
        for cn, r in results.items():
            d = (r['avg_time'] - c5_t) / c5_t * 100
            sa = f"{r.get('actual_s_avg', 0):.0f}" if 'actual_s_avg' in r else "—"
            ki = f"{r.get('k_init_avg', 0):.0f}" if 'k_init_avg' in r else "—"
            print(f"  {cn:<35s} {r['avg_time']:>8.3f} {r['avg_fwd']:>5.0f} "
                  f"{r['ms_per_fwd']:>8.2f} {d:>+7.1f}% {sa:>8s} {ki:>8s}")

        out_path = REPO_ROOT / "codex_coding" / "results" / "c10_v3_comparison.json"
