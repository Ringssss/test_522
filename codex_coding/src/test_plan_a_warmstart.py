#!/usr/bin/env python3
"""
v0.1.15.7c — Plan A: Per-Forward Warm-Start Batch-Add

Phase 1: Validate algorithm in Python. Key question: does per-forward
warm-start batch-add keep fwd count at ~272 (vs 285 with per-block cache)?

Configs:
  C5:  Fused routing only (re-confirm ~7.77s, 272 fwd)
  C7a: Plan A — per-forward warm-start batch-add on all layers
       K_target=40, M=4, QF=0.70, q_major=0.95, cap=8
"""
...
# ================================================================
# Fused Routing Kernel (from test_fused_routing.py, v2 with S_mask)
# ================================================================
@triton.jit
...
# ================================================================
# Plan A: Warm-Start Batch-Add Controller
# ================================================================
class WarmStartEBController:
    """Per-forward, per-layer warm-start batch-add."""
    ...
    def __init__(self, K=8, M=4, K_target=40, quality_floor=0.70,
        ...
        # Per-layer warm-start state
        self.prev_active_count = {}  # layer_idx → int
        # Stats
        self.total_rounds = 0
        self.total_calls = 0
        self.cold_starts = 0
    ...
    def compute_s_mask(self, layer_idx, logits, expert_bias, rsf):
        N, E = logits.shape
        ...
        # Init S — warm start or cold start
        if layer_idx in self.prev_active_count:
            K_init = self.prev_active_count[layer_idx]
        else:
            K_init = self.K_target
            self.cold_starts += 1
        ...
        active = int(S_mask.sum().item())
        self.prev_active_count[layer_idx] = active
        self.total_rounds += n_rounds
        self.total_calls += 1
        self.rounds_hist.append(n_rounds)

        return S_mask.int(), active

    def summary(self):
        if self.total_calls == 0:
            return {}
        avg_rounds = self.total_rounds / self.total_calls
        avg_active = sum(self.prev_active_count.values()) / max(len(self.prev_active_count), 1)
        zero_round_pct = sum(1 for r in self.rounds_hist if r == 0) / len(self.rounds_hist) * 100
        return {
            "total_calls": self.total_calls,
            "cold_starts": self.cold_starts,
            "avg_rounds": avg_rounds,
            "avg_active": avg_active,
            "zero_round_pct": zero_round_pct,
            "rounds_distribution": {
                str(i): sum(1 for r in self.rounds_hist if r == i)
                for i in range(max(self.rounds_hist) + 1) if sum(1 for r in self.rounds_hist if r == i) > 0
            },
        }
...
def main():
    ...
    print("=" * 80)
    print("v0.1.15.7c — Plan A: Per-Forward Warm-Start Batch-Add")
    print("  Key question: does per-fwd warm-start keep fwd count at ~272?")
    print("=" * 80)
    ...
    with set_current_vllm_config(VllmConfig(parallel_config=pcfg)):
        ...
        print(f"  Avg: {c5_avg:.3f}s, {c5_fwd:.0f} fwd, {c5_fwd/c5_avg:.1f} fwd/s, {c5_avg/c5_fwd*1000:.2f} ms/fwd")
        restore()

        # ============================================================
        # C7a: Plan A — per-forward warm-start batch-add
        # ============================================================
        print(f"\n{'='*60}")
        print("C7a: Plan A — per-forward warm-start batch-add")
        print("  K_target=40, M=4, QF=0.70, q_major=0.95, cap=8")
        print(f"{'='*60}")

        ctrl = WarmStartEBController(K=8, M=4, K_target=40, quality_floor=0.70,
        ...
        for name, mod in model.named_modules():
            if mod.__class__.__name__ == "LLaDA2MoeGate":
                ...
                def mk(bb, rr, tt, nn, gg, layer_i, cc):
                    def fn(hs, go, topk, renorm):
                        sm, _ = cc.compute_s_mask(layer_i, go, bb, rr)
        ...
        warmup_summary = ctrl.summary()
        print(f"  Warmup stats: avg_rounds={warmup_summary.get('avg_rounds',0):.2f}, "
              f"zero_round_pct={warmup_summary.get('zero_round_pct',0):.1f}%, "
              f"avg_active={warmup_summary.get('avg_active',0):.1f}")
        print(f"  Rounds distribution: {warmup_summary.get('rounds_distribution', {})}")

        # Reset stats for measurement runs
        ctrl.total_rounds = 0
        ctrl.total_calls = 0
        ctrl.cold_starts = 0
        ctrl.rounds_hist = []
        ...
        c7_times, c7_fwds = [], []
        for ri in range(2):
            # Reset per-layer warm state (simulate fresh start with warm knowledge)
            # Keep prev_active_count (warm state), just reset stats
            ctrl.total_rounds = 0
            ctrl.total_calls = 0
            ctrl.cold_starts = 0
            ctrl.rounds_hist = []
            ...
            torch.cuda.synchronize()
            t1 = time.perf_counter()
            c7_times.append(t1 - t0)
            c7_fwds.append(dllm.diff_iteration.num_forwards)
            print(f"  Run {ri+1}: {t1-t0:.3f}s, {dllm.diff_iteration.num_forwards} fwd")

        c7_avg = sum(c7_times) / len(c7_times)
        c7_fwd = sum(c7_fwds) / len(c7_fwds)
        delta_c5 = (c7_avg - c5_avg) / c5_avg * 100
        c0_ref = 8.792
        delta_c0 = (c7_avg - c0_ref) / c0_ref * 100

        run_summary = ctrl.summary()
        print(f"\n  C7a avg: {c7_avg:.3f}s, {c7_fwd:.0f} fwd, {c7_fwd/c7_avg:.1f} fwd/s, "
              f"{c7_avg/c7_fwd*1000:.2f} ms/fwd")
        print(f"  vs C5: {delta_c5:+.1f}%  vs C0: {delta_c0:+.1f}%")
        print(f"  Fwd count: C5={c5_fwd:.0f} → C7a={c7_fwd:.0f} (delta={c7_fwd-c5_fwd:+.0f})")
        print(f"  EB stats: avg_rounds={run_summary.get('avg_rounds',0):.2f}, "
              f"zero_round_pct={run_summary.get('zero_round_pct',0):.1f}%, "
              f"avg_active={run_summary.get('avg_active',0):.1f}")
        print(f"  Rounds distribution: {run_summary.get('rounds_distribution', {})}")

        results["C7a"] = {
            "avg_time": c7_avg, "avg_fwd": c7_fwd,
            "fwd_per_s": c7_fwd/c7_avg, "ms_per_fwd": c7_avg/c7_fwd*1000,
            "delta_vs_c5_pct": delta_c5, "delta_vs_c0_pct": delta_c0,
            "eb_stats": run_summary,
        }
        ...
        print("SUMMARY")
        print(f"{'='*80}")
        print(f"  C0 reference: 8.792s, 262 fwd, 29.8 fwd/s")
        print(f"  {'Config':<20s} {'Time':>7s} {'Fwd':>5s} {'Fwd/s':>6s} {'ms/fwd':>7s} {'vs C5':>7s} {'vs C0':>7s}")
        print(f"  {'-'*65}")
        for cn, r in results.items():
            ...
            print(f"  {cn:<20s} {r['avg_time']:>7.3f} {r['avg_fwd']:>5.0f} {r['fwd_per_s']:>6.1f} "
                  f"{r['ms_per_fwd']:>7.2f} {d5:>7s} {d0:>7s}")

        out_path = REPO_ROOT / "codex_coding" / "results" / "plan_a_warmstart_results.json"
