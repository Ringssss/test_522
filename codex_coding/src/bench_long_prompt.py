#!/usr/bin/env python3
"""
Long-prompt benchmark: compare dInfer paths with prompt > 512 tokens.
Includes IterationSmooth variants.
"""
...
# Long prompt: concatenate multiple reasoning problems to push past 512 tokens
...
def main() -> None:
    ...
    with set_current_vllm_config(VllmConfig(parallel_config=parallel_config)):
        ...
        assert prompt_len >= 512, f"Prompt too short: {prompt_len} tokens, need >= 512"

        # Model kernel warmup
        warmup_x = torch.arange(prompt_len + GEN_LENGTH, dtype=torch.long, device=device).unsqueeze(0)
        with torch.inference_mode():
        ...
        # === A. BlockDiffusionLLM + prefix cache (previous best) ===
        ...
        # === B. BlockDiffusionLLMAttnmask (no KV cache) ===
        dllm = BlockDiffusionLLMAttnmask(
            model, make_decoder(),
            BlockIteratorFactory(use_block_diffusion=True),
            early_stop=True,
        )
        all_results.append(bench_dllm("B: BD attnmask (no cache)", dllm, input_ids, device))
        ...
        # === E. IterSmoothDiffusionLLM + prefix cache ===
        ...
        all_results.append(bench_dllm("E: IterSmooth + prefix", dllm, input_ids, device))

        # === F. VicinityCacheDiffusionLLM ===
        ...
        all_results.append(bench_dllm("F: VicinityCache (look=32)", dllm, input_ids, device))

        # === G. IterSmoothWithVicinityCacheDiffusionLLM ===
        ...
        all_results.append(bench_dllm("G: IterSmooth+Vicinity", dllm, input_ids, device))
    ...
    print("\n" + "=" * 90)
    print(f"LONG PROMPT BENCHMARK (prompt={prompt_len} tok, gen={GEN_LENGTH}, block={BLOCK_LENGTH}, threshold={THRESHOLD})")
    print("=" * 90)
    ...
    best = all_results[0]
    print(f"\nFastest: {best['name']}")
    print(f"  -> {best['tok/s']:.1f} tok/s, {best['fwd/s']:.1f} fwd/s, {best['forwards']} forwards")

    output_path = RESULTS_DIR / "long_prompt_benchmark_results.json"
