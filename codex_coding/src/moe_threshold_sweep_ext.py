#!/usr/bin/env python3
"""
v0.1.14.8b — Extended threshold sweep: explore 0.7, 0.6, 0.4, 0.3, 0.2
Append to the previous sweep data. Same setup: batch=32, L4-14, temp=0 x5 runs + temp=0.7 output.
"""
...
class SweepController:
    def __init__(self, margin_threshold=0.99):
    ...
    def hook_forward(self, moe_mod, mi, hidden_states):
        ...
        n_pos = bsz * seq_len
        self.total += n_pos
        self.per_step_total[self.step] += n_pos
        ...
        if (mi in self.reuse_layers and self.qualifying_mask is not None
            ...
            if cached.shape[0] >= bsz and cached.shape[1] == seq_len:
                ...
                if n > 0:
                    ...
                    self.reused += n
                    self.per_step_reused[self.step] += n
...
def main():
    ...
    print("=" * 80)
    print("v0.1.14.8b — Extended Threshold Sweep (0.7, 0.6, 0.4, 0.3, 0.2)")
    print("=" * 80)
    ...
    with set_current_vllm_config(VllmConfig(parallel_config=pcfg)):
        ...
        THRESHOLDS = [0.70, 0.60, 0.40, 0.30, 0.20]
        N_RUNS = 5

        # ---- PART 1: temp=0, quantitative ----
        print(f"\n{'='*80}")
        print(f"PART 1: Quantitative (temp=0, {N_RUNS} runs)")
        print(f"{'='*80}")
        ...
        print(f"\n  Baseline: fwds=[{','.join(str(f) for f in bl_fwds)}] avg={bl_avg:.0f}")

        results = {}
        for thresh in THRESHOLDS:
            fwds_list, reuse_pcts = [], []
            last_ctrl = None
            for r in range(N_RUNS):
                ctrl = SweepController(margin_threshold=thresh)
            ...
            print(f"\n  margin>{thresh}: fwds=[{','.join(str(f) for f in fwds_list)}] "
                  f"avg={avg_fwd:.0f} (Δ={delta:+.0f}) reuse={avg_reuse:.1f}%")
        ...
        print(f"\n{'='*80}")
        print(f"FULL SWEEP SUMMARY (combined with v0.1.14.8a)")
        print(f"{'='*80}")
        print(f"  {'Threshold':>10s} {'Reuse%':>7s} {'AvgFwd':>7s} {'ΔFwd':>6s} {'Theory%':>8s} {'Verdict':>10s}")
        print(f"  {'-'*55}")

        # Include previous results manually
        prev = {
            "baseline": (0.0, 149, 0),
            0.999: (11.1, 152, +3),
            0.99: (16.5, 148, -1),
        ...
        all_points = []
        for k, (rp, af, d) in prev.items():
            all_points.append((k, rp, af, d))
        for thresh in THRESHOLDS:
            r = results[thresh]
            all_points.append((thresh, r["avg_reuse_pct"], r["avg_fwd"], r["delta"]))

        # Sort by threshold descending
        all_points.sort(key=lambda x: (isinstance(x[0], str), -x[0] if isinstance(x[0], (int, float)) else 0))

        for item in all_points:
            k, rp, af, d = item
            theory = rp * 0.65 * 0.55 / 100
            if isinstance(k, str):
                label = k
            else:
                label = f">{k}"
            ...
            print(f"  {label:>10s} {rp:>6.1f}% {af:>7.0f} {d:>+5.0f} {theory*100:>7.1f}% {verdict:>10s}")
        ...
        for thresh in CHECK:
            ...
            try:
                ...
                thresh_outs[thresh] = out[:, prompt_len:].cpu()
            finally:
                remove_hooks(hooks)
