#!/usr/bin/env python3
"""
v0.1.15.8f — C10-v6: Original K_A + K_B_v3 (tl.sort)

Only change vs C10-v3: K_B_v2 (iterative argmax) → K_B_v3 (tl.sort)
K_A and K_init logic identical.
"""
...
# ================================================================
# K_B_v3: tl.sort based (from warmstart_triton_final.md)
# ================================================================
@triton.jit
def _kernel_B_v3(
...
# ================================================================
# V6 Controller: original K_A + K_B_v3 + K_init from cold path
# ================================================================
class FusedEBControllerV6(FusedEBController):
    ...
    def hot_path(self, layer_idx, logits, bias):
        """Original K_A + new K_B_v3 (tl.sort)."""
        N, E = logits.shape
        K_init = self.K_init.get(layer_idx, 103)
        b = self._get_bufs(N, E, logits.device)

        lf = logits.float()
        # Original K_A (per-token atomic)
...
def main():
    ...
    print("=" * 80)
    print("v0.1.15.8f — C10-v6: Original K_A + K_B_v3 (tl.sort)")
    ...
    with set_current_vllm_config(VllmConfig(parallel_config=pcfg)):
        ...
        print(f"\n{'='*60}")
        print("C10-v3: original K_A + K_B_v2 (iterative argmax)")
        print(f"{'='*60}")
        ...
        # C10-v6: original K_A + K_B_v3 (tl.sort) + actual |S|
        print(f"\n{'='*60}")
        print("C10-v6: original K_A + K_B_v3 (tl.sort)")
        print(f"{'='*60}")
        ctrl_v6 = FusedEBControllerV6(
            num_layers=19, K=8, M=4, K_target=40,
            quality_floor=0.70, q_major=0.95, per_round_cap=8)
        results['C10_v6'] = run_config("C10-v6", ctrl_v6)
        ...
        out_path = REPO_ROOT / "codex_coding" / "results" / "c10_v6_comparison.json"
