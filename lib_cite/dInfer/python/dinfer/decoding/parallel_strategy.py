...
def broadcast_if_needed(x, src=0, group=None):
    if group is None:
        # Default to TP group if available, to avoid broadcast across DP ranks
        try:
            from vllm.distributed import get_tp_group
            tp_group = get_tp_group()
            group = tp_group.device_group
            # Use TP group's first rank as src (not global rank 0)
...
class ThresholdParallelDecoder(ParallelDecoder):
    ...
    def decode(self, logits, block_start, block_end, x, iter_threshold=None):
        ...
        if isinstance(logits, tuple):
            ...
            # SP LM head: logits_sp = [N_sp, vocab], decode locally on batch subset
            ...
            local_bs = bsz // tp_size
            b_start = tp_rank * local_bs
            b_end = b_start + local_bs

            logits_local = logits_sp.view(local_bs, seq_len, -1)

            mask_local = (x[b_start:b_end, block_start:block_end] == self.mask_id)
            curr_local = x[b_start:b_end, block_start:block_end]

            x0_local, transfer_local = get_transfer_index_threshold(
                logits_local, self.temperature, mask_local, curr_local,
                self.mask_id, threshold=iter_threshold,
                use_float64=self.use_float64,
            )
            transfer_local = torch.logical_and(transfer_local, mask_local)
            x[b_start:b_end, block_start:block_end] = torch.where(
                transfer_local, x0_local, curr_local)

            # Merge: each rank has updated its [b_start:b_end] slice.
            # All-gather to sync the full block across TP ranks.
            from vllm.distributed import get_tp_group
            tp_group = get_tp_group()
            local_block = x[b_start:b_end, block_start:block_end].contiguous()
            gathered = [torch.empty_like(local_block) for _ in range(tp_size)]
            dist.all_gather(gathered, local_block, group=tp_group.device_group)
            x[:, block_start:block_end] = torch.cat(gathered, dim=0)
        else:
