...
class BlockDiffusionRunner(BlockRunner):
    ...
    def decode(self, model, decoder, x, kv_cache, block, block_loc, block_id, pos_ids, attn_mask, block_length=32, cross_block_attn_mask=None):
        ...
        # Check if running in DP mode (dp_size > 1)
        _dp_size = 1
        _dp_group = None
        try:
            from vllm.config import get_current_vllm_config
            _cfg = get_current_vllm_config()
            if _cfg is not None and _cfg.parallel_config.data_parallel_size > 1:
                _dp_size = _cfg.parallel_config.data_parallel_size
                from vllm.distributed import get_dp_group
                _dp_group = get_dp_group().device_group
        except Exception:
            pass

        while True:
            # Global convergence check: all DP ranks must agree to stop
            local_remaining = (block == decoder.mask_id).sum()
            if _dp_size > 1:
                global_remaining = local_remaining.clone()
                import torch.distributed as _dist
                _dist.all_reduce(global_remaining, op=_dist.ReduceOp.MAX,
                                 group=_dp_group)
                if global_remaining.item() == 0:
                    break
            else:
                if local_remaining.item() == 0:
                    break

            # In DP mode, force unroll_k=1 to ensure all ranks call forward()
            # the same number of times (AllToAll collective requires alignment)
            if _dp_size > 1:
                unroll_k = 1
            else:
                unroll_k = int(max(min((block == decoder.mask_id).sum()//self.expected_tpf, self.maximum_unroll), 1))
                ...
                if self.need_cross_block_update:
                    ...
                    if self.backend=='vllm':
                        if isinstance(output.past_key_values, list):
                            ...
                            # Ensure _data is a consolidated tensor before slicing
            ...
            # In DP mode, disable mid-block batch shrinking to keep batch size uniform
            if batch_size > 1 and _dp_size <= 1:
                seq_idx, x = select_undecoded(seq_idx, orig_x, x, block, block_loc, decoder.mask_id, writeback=True)
                block = x[:, block_loc.start:block_loc.end]
                # If all blocks have been decoded, we can jump out.
                if len(seq_idx) == 0:
                    break
            elif batch_size > 1 and _dp_size > 1:
                # DP mode: just writeback decoded tokens but keep all sequences
                orig_x[:, block_loc.start:block_loc.end] = x[:, block_loc.start:block_loc.end]
...
class BlockDiffusionIteration:
    ...
    """
    def __init__(self, lazy_cache_update=False, inplace_cache_update=False):
        self.num_forwards = 0
        self.cache_updates = 0
        self.iter_no = 0
        self.lazy_cache_update = lazy_cache_update
        self.inplace_cache_update = inplace_cache_update
    ...
    def forward(self, model, decoder, x, kv_cache, block, block_loc, block_id, pos_ids, attn_mask, past_key_values, replace_position, backend, is_cross_block=False, block_length=32):
        ...
        if kv_cache is None:
            ...
            # Enable in-place KV cache writes to avoid per-layer slice_scatter allocation
            if self.inplace_cache_update and hasattr(past_key_values, 'use_inplace_update'):
            ...
            if backend == 'vllm' and not self.lazy_cache_update:
                kv_cache.update(output.past_key_values)
        if is_cross_block:
            if isinstance(logits, tuple):
                logits_sp, sp_state = logits
                from vllm.distributed import get_tensor_model_parallel_world_size
                tp = get_tensor_model_parallel_world_size()
                local_bs = sp_state.bsz // tp
                logits_sp_3d = logits_sp.view(local_bs, sp_state.seq_len, -1)
                logits_sp_sliced = logits_sp_3d[:, block_length:, :].reshape(-1, logits_sp_3d.shape[-1])
                from dinfer.model.modeling_llada2_moe import _BSPGSPHiddenState
                sp_state_sliced = _BSPGSPHiddenState(
                    None, sp_state.bsz, sp_state.seq_len - block_length,
                    sp_state.bsz * (sp_state.seq_len - block_length))
                logits = (logits_sp_sliced, sp_state_sliced)
            else:
                logits = logits[:, block_length:]
            decoder.decode(logits, block_loc.start+block_length, block_loc.end, x)
        else: 
            decoder.decode(logits, block_loc.start, block_loc.end, x)
        self.num_forwards += 1
        self.iter_no += 1
...
class VicinityCacheIteration(DiffusionIteration):
    ...
    def forward(self, model, decoder, x, kv_cache, block, block_loc, block_id):
        ...
        past_key_values, _ = kv_cache.get_key_values(left_start, right_end)
        window_replace = (int(left_start), int(right_end))
        window_pos_ids = torch.arange(left_start, right_end, device=x.data.device, dtype=torch.long).unsqueeze(0)
        out_step = model(window_input, past_key_values=past_key_values, use_cache=True, replace_position=window_replace, position_ids=window_pos_ids)
        self.num_forwards += 1
...
class IterSmoothWithVicinityCache(DiffusionIteration):
    ...
    def forward(self, model, decoder, x, kv_cache, block, block_loc, block_id):
        ...
        out_step = model(
                ...
                past_key_values=past_key_values,
                use_cache=True,
                replace_position=window_replace,
                position_ids=window_pos_ids
        )
...
class BlockDiffusionLLMAttnmask(DiffusionLLM):
    ...
    def generate(self, prompt, gen_length=128, block_length=128):
        ...
        bd_attn_mask = block_mask.repeat_interleave(block_length, dim=0)\
                                        .repeat_interleave(block_length, dim=1).unsqueeze(0).expand(batch_size, -1, -1)
        pos_ids = torch.arange(total_length, device=self.model.device).unsqueeze(0).expand(batch_size, -1)
...
class IterSmoothBlockDiffusionIteration:
    """ Block diffusion iteration with IterSmooth soft embedding.

    Only applies soft embedding to mask positions within the current block,
    keeping prompt and completed blocks as discrete embeddings.
    """
    def __init__(self, h2e, cont_weight=0.3, cont_weight_init=0.15,
    ...
    def init_embeds(self, x):
        """ Initialize inputs_embeds from discrete token ids. """
        self.inputs_embeds = self.h2e(x.data)

    def forward(self, model, decoder, x, kv_cache, block, block_loc, block_id,
                pos_ids, attn_mask, past_key_values, replace_position, backend,
                is_cross_block=False, block_length=32):
        """ Forward with soft embedding on current block mask positions. """
        iter_cont_weight = min(
            self.cont_weight_init + self.cont_weight_growth * self.iter_no,
            self.cont_weight)
        iter_threshold = max(
            1 - self.iter_no * self.threshold_decay, decoder.threshold)

        if kv_cache is None:
            # No-cache path: send full prefix + current block with attnmask
            output = model(
                inputs_embeds=self.inputs_embeds[:, :block_loc.end],
                attention_mask=attn_mask[:, :block_loc.end, :block_loc.end],
                position_ids=pos_ids[:, :block_loc.end])
            logits = output.logits[:, block_loc.start:block_loc.end]
        else:
            # Cache path: send only current block embeddings with past_key_values
            block_embeds = self.inputs_embeds[:, block_loc.start:block_loc.end] \
        ...
        # Decode tokens using dynamic threshold
        decoder.decode(logits, block_loc.start, block_loc.end, x, iter_threshold)

        # Update embeddings: soft for mask positions in current block, discrete elsewhere
        block_mask_index = (x.data[:, block_loc.start:block_loc.end] == decoder.mask_id)
        self.inputs_embeds[:, block_loc.start:block_loc.end] = self.h2e(
            x.data[:, block_loc.start:block_loc.end],
            block_mask_index, logits, iter_cont_weight)
...
class IterSmoothBlockDiffusionLLMAttnmask(DiffusionLLM):
    """ BD-attnmask + IterSmooth: soft embedding only on current block mask positions.

    Combines the BD-attnmask path (truncated view, block-causal attention mask,
    no KV cache) with IterSmooth's soft embedding, but restricted to mask
    positions within the current block only.
    """
    def __init__(self, model, decoder, iterator_factory, early_stop=True,
                 maximum_unroll=4, expected_tpf=8, backend='vllm',
                 cont_weight=0.3, cont_weight_init=0.15,
                 cont_weight_growth=0.02, threshold_decay=0.02):
        self.model = model
        self.decoder = decoder
        self.iterator_factory = iterator_factory
        # Extract h2e from model
        if isinstance(model, torch.nn.parallel.DistributedDataParallel):
            h2e = model.module.h2e
        else:
            h2e = model.h2e
        self.diff_iteration = IterSmoothBlockDiffusionIteration(
            h2e, cont_weight, cont_weight_init, cont_weight_growth, threshold_decay)
        self.block_runner = BlockDiffusionRunner(
            self.diff_iteration, early_stop, maximum_unroll, expected_tpf, backend)
    ...
    @torch.no_grad()
    def generate(self, prompt, gen_length=128, block_length=128):
        """ Generate tokens with BD-attnmask + IterSmooth. """
        batch_size = prompt.shape[0]
        prompt_length = prompt.shape[1]
        num_blocks = (prompt_length + gen_length + block_length - 1) // block_length
        total_length = num_blocks * block_length
        ...
        # Prepare block-causal attention mask and position IDs
        block_mask = torch.tril(torch.ones(num_blocks, num_blocks, device=self.model.device))
        bd_attn_mask = block_mask.repeat_interleave(block_length, dim=0) \
                                 .repeat_interleave(block_length, dim=1).unsqueeze(0).expand(batch_size, -1, -1)
        pos_ids = torch.arange(total_length, device=self.model.device).unsqueeze(0).expand(batch_size, -1)

        x = TokenArray(prompt, new_gen_length, self.decoder.mask_id,
                       self.decoder.eos_id, self.model.device)
        it = self.iterator_factory.create(x, block_length)

        # Reset iteration state and initialize discrete embeddings
        self.diff_iteration.iter_no = 0
        self.diff_iteration.init_embeds(x)

        kv_cache = None
        for block_id, (block_loc, block) in enumerate(it):
            self.decoder.block_init(block, block_id)
            decode_compl = self.block_runner.decode(
                self.model, self.decoder, x, kv_cache, block, block_loc,
                block_id, pos_ids, bd_attn_mask)
            # After block completes, refresh its embeddings back to discrete
            self.diff_iteration.inputs_embeds[:, block_loc.start:block_loc.end] = \
                self.diff_iteration.h2e(x.data[:, block_loc.start:block_loc.end])
            if decode_compl.all():
                break
        logger.info(f'The number of diffusion iterations: {self.num_forwards}')
        return x.get_generated_tokens()
...
class IterSmoothBlockDiffusionLLMCache(DiffusionLLM):
    """ BD + IterSmooth + KV cache: soft embedding with cached prefix KV.

    Same as IterSmoothBlockDiffusionLLMAttnmask but uses KV cache to avoid
    recomputing prefix KV at every step. Each decode step only sends the
    current block's inputs_embeds (32 tokens) with cached past_key_values.
    """
    def __init__(self, model, decoder, iterator_factory, cache_factory,
                 early_stop=True, maximum_unroll=1, expected_tpf=8,
                 backend='vllm', prefilling_limit=128,
                 cont_weight=0.3, cont_weight_init=0.15,
                 cont_weight_growth=0.02, threshold_decay=0.02):
        self.model = model
        self.decoder = decoder
    ...
    @torch.no_grad()
    def generate(self, prompt, gen_length=128, block_length=128):
        """ Generate tokens with BD + IterSmooth + KV cache. """
        batch_size = prompt.shape[0]
        prompt_length = prompt.shape[1]
        num_blocks = (prompt_length + gen_length + block_length - 1) // block_length
        total_length = num_blocks * block_length
        ...
        mask_length = (max(self.cache_factory.max_length, prompt_length + gen_length) +
                       block_length - 1) // block_length * block_length
        attn_mask_num_blocks = mask_length // block_length

        # Prepare block-causal attention mask and position IDs
        block_mask = torch.tril(torch.ones(
            attn_mask_num_blocks, attn_mask_num_blocks,
            device=self.model.device, dtype=torch.bool))
        bd_attn_mask = block_mask.repeat_interleave(block_length, dim=0) \
                                 .repeat_interleave(block_length, dim=1) \
                                 .unsqueeze(0).repeat(batch_size, 1, 1)
        pos_ids = torch.arange(total_length, device=self.model.device) \
        ...
        # Prefill prompt KV cache (uses input_ids, discrete embeddings)
        prefill_blocks = prompt_length // block_length
        prefill_length = prefill_blocks * block_length
        prefill_length = max(prefill_length, block_length)
        self.block_runner.prefill(
            self.model, x[:, :prefill_length], kv_cache,
            pos_ids[:, :prefill_length],
            bd_attn_mask[:, :prefill_length, :prefill_length],
            self.prefilling_limit, block_length)

        # Initialize inputs_embeds for soft embedding
        self.diff_iteration.iter_no = 0
        self.diff_iteration.init_embeds(x)
        ...
        for block_id, (block_loc, block) in enumerate(it):
            self.decoder.block_init(block, block_id)
            if self.backend == 'vllm':
                cross_block_attn_mask = bd_attn_mask[
                    :, block_loc.start - block_length:block_loc.end, :block_loc.end]
            else:
                cross_block_attn_mask = None
            decode_compl = self.block_runner.decode(
                self.model, self.decoder, x, kv_cache, block, block_loc,
                block_id, pos_ids, bd_attn_mask, block_length,
...
class StableCacheBlockDiffusionIteration:
    """ Block diffusion iteration with Stable Cache v2 MoE optimization.

    v2 strategy (empirically validated for exact match):
      - Layer 18 (sensitive layer): always full computation, never cached
      - Other 18 MoE layers: stable positions use cached routed-expert output
      - Every refresh_interval steps: full computation on ALL layers (safety net)
      - Attention always runs on all positions (bidirectional, cannot skip)

    "Stable" = position whose token has not changed since the previous iteration.
    """
    SENSITIVE_LAYER = 18       # layer index that must always compute fully
    REFRESH_INTERVAL = 2       # full refresh every N iterations (keeps cache at most 1-step old)

    def __init__(self, model, lazy_cache_update=False, inplace_cache_update=False):
        self.num_forwards = 0
        self.cache_updates = 0
        self.iter_no = 0
        self.lazy_cache_update = lazy_cache_update
        ...
        # stable cache state (reset per block)
        self.prev_block_tokens = None        # [batch, block_len]
        self.stable_mask = None              # [batch, block_len] True = stable
        self.moe_output_cache = {}           # layer_idx -> [batch, block_len, hidden]
        self._layer_counter = 0
        self._iter_in_block = 0              # iteration count within current block

        # collect MoE layers from the model for patching
        self._moe_layers = []
        self._original_forwards = {}
        layers = model.model.layers if not isinstance(model, torch.nn.parallel.DistributedDataParallel) else model.module.model.layers
        for layer_idx, layer in enumerate(layers):
            if hasattr(layer, 'mlp') and hasattr(layer.mlp, 'gate'):
                self._moe_layers.append((layer_idx, layer.mlp))

    def reset_block(self):
        """ Clear cache at block boundaries. """
        self.prev_block_tokens = None
        self.stable_mask = None
        self.moe_output_cache = {}
        self._iter_in_block = 0

    def _compute_stable_mask(self, block_tokens):
        """ Compare current block tokens with previous iteration. """
        if self.prev_block_tokens is None:
            self.stable_mask = None
        else:
    ...
    def _is_refresh_step(self):
        """ Whether this iteration should do full computation (periodic refresh). """
        return self._iter_in_block % self.REFRESH_INTERVAL == 0

    def _make_selective_forward(self, layer_idx, moe_module):
        """ Create a selective MoE forward for one layer.

        Caches the FULL MoE output (shared + routed) so that stable positions
        get a consistent snapshot rather than a mix of fresh-shared + stale-routed.
        """
        stable_mask = self.stable_mask
        moe_cache = self.moe_output_cache   # stores FULL output (shared+routed)
        is_sensitive = (layer_idx == self.SENSITIVE_LAYER)
        is_refresh = self._is_refresh_step()
        parent = self

        def selective_forward(hidden_states):
            bsz, seq_len, h = hidden_states.shape
            cur_layer = parent._layer_counter
            parent._layer_counter += 1

            # Determine if this layer should use cache
            use_cache_this_layer = (
                not is_sensitive
                and not is_refresh
                and stable_mask is not None
                and stable_mask.any()
                and cur_layer in moe_cache
            )

            if not use_cache_this_layer:
                # Full computation: shared + routed on ALL positions, cache full output
                res = moe_module.shared_experts(hidden_states)
                hs_flat = hidden_states.view(-1, h)
                router_logits = moe_module.gate.get_logits(hs_flat)
                y = moe_module.experts.forward_impl(hidden_states=hs_flat, router_logits=router_logits)
                y = y.view(bsz, seq_len, h)
                full_out = y + res if moe_module.config.num_shared_experts is not None else y
                moe_cache[cur_layer] = full_out.detach().clone()
                return full_out

            # Selective computation: only active positions
            any_active = (~stable_mask).any(dim=0)    # [seq_len]
            active_idx = any_active.nonzero(as_tuple=True)[0]
            n_active = len(active_idx)

            if n_active == 0:
                # all stable: pure cache (full output from prev iteration)
                return moe_cache[cur_layer].clone()

            if n_active == seq_len:
                # all active: normal full path
                res = moe_module.shared_experts(hidden_states)
                hs_flat = hidden_states.view(-1, h)
                router_logits = moe_module.gate.get_logits(hs_flat)
                y = moe_module.experts.forward_impl(hidden_states=hs_flat, router_logits=router_logits)
            ...
            # Partial active: extract active, compute shared+routed, scatter back
            active_hs = hidden_states[:, active_idx, :]                # [bsz, n_active, h]

            # Shared expert on active positions only
            active_shared = moe_module.shared_experts(active_hs)

            # Routed experts on active positions only
            active_flat = active_hs.reshape(bsz * n_active, h)
            router_logits = moe_module.gate.get_logits(active_flat)
            y_active_routed = moe_module.experts.forward_impl(
                hidden_states=active_flat, router_logits=router_logits)
            y_active_routed = y_active_routed.view(bsz, n_active, h)

            # Combine shared + routed for active positions
            if moe_module.config.num_shared_experts is not None:
                active_full = y_active_routed + active_shared
            else:
                active_full = y_active_routed

            # Merge: stable from cached full, active from fresh full
            full_out = moe_cache[cur_layer].clone()
            full_out[:, active_idx, :] = active_full
            moe_cache[cur_layer] = full_out.detach().clone()
            return full_out

        return selective_forward

    def _patch_moe_layers(self, model):
        """ Install selective MoE forwards for all MoE layers. """
        self._layer_counter = 0
        for layer_idx, moe_module in self._moe_layers:
            self._original_forwards[layer_idx] = moe_module.forward
            moe_module.forward = self._make_selective_forward(layer_idx, moe_module)

    def _unpatch_moe_layers(self, model):
        """ Restore original MoE forwards. """
        for layer_idx, moe_module in self._moe_layers:
            if layer_idx in self._original_forwards:
                moe_module.forward = self._original_forwards[layer_idx]
        self._original_forwards.clear()

    def forward(self, model, decoder, x, kv_cache, block, block_loc, block_id,
                pos_ids, attn_mask, past_key_values, replace_position, backend,
                is_cross_block=False, block_length=32):
        """ Forward with stable cache v2 MoE optimization. """
        if not is_cross_block:
            block_tokens = x.data[:, block_loc.start:block_loc.end]
            self._compute_stable_mask(block_tokens)
        ...
        if kv_cache is None:
            ...
            # Install selective MoE for non-cross-block cache-path forwards
            # that have stable positions and are not the first iteration
            use_selective = (not is_cross_block and self.stable_mask is not None)
            if use_selective:
                self._patch_moe_layers(model)
            ...
            if use_selective:
                self._unpatch_moe_layers(model)
...
class StableCacheBlockDiffusionLLM(DiffusionLLM):
    """ Block Diffusion LLM with Stable Cache MoE optimization.

    Uses KV cache for prefix + selective MoE computation that skips
    stable (unchanged) positions and reuses cached expert output.
    """
    def __init__(self, model, decoder, iterator_factory, cache_factory,
                 early_stop=True, maximum_unroll=1, expected_tpf=15, backend='vllm',
        ...
        self.iterator_factory = iterator_factory
        self.cache_factory = cache_factory
        self.diff_iteration = StableCacheBlockDiffusionIteration(
            model, lazy_cache_update=lazy_cache_update,
            inplace_cache_update=inplace_cache_update)
        self.block_runner = BlockDiffusionRunner(
            self.diff_iteration, early_stop, maximum_unroll, expected_tpf, backend)
        self.early_stop = early_stop
        self.backend = backend
    ...
    def naive_batching_generate(self, prompt, gen_length=128, block_length=128):
        ...
        for block_id, (block_loc, block) in enumerate(it):
            self.decoder.block_init(block, block_id)
            # reset stable cache at each new block
            self.diff_iteration.reset_block()
            if self.backend == 'vllm':
                cross_block_attn_mask = bd_attn_mask[
                    :, block_loc.start - block_length:block_loc.end, :block_loc.end]
            else:
                cross_block_attn_mask = torch.ones(
                    batch_size, 2 * block_length,
                    kv_cache.past_key_values._data.shape[4],
                    device=prompt.device, dtype=torch.bool)
                cross_block_attn_mask[:, :block_length, -block_length:].fill_(False)
            decode_compl = self.block_runner.decode(
                self.model, self.decoder, x, kv_cache, block, block_loc,
                block_id, pos_ids, bd_attn_mask, block_length, cross_block_attn_mask)
...
class BlockDiffusionLLM(DiffusionLLM):
    ...
    def __init__(self, model, decoder, iterator_factory, cache_factory, early_stop=True, maximum_unroll=1, expected_tpf=15, backend='vllm',
                 mini_batch_size=4, prefilling_limit=128, use_naive_batching=True,
                 lazy_cache_update=False, inplace_cache_update=False):
        self.model = model
        self.decoder = decoder
        self.iterator_factory = iterator_factory
        self.cache_factory = cache_factory
        self.diff_iteration = BlockDiffusionIteration(lazy_cache_update=lazy_cache_update,
                                                       inplace_cache_update=inplace_cache_update)
        self.block_runner = BlockDiffusionRunner(self.diff_iteration, early_stop, maximum_unroll, expected_tpf, backend)
        self.early_stop = early_stop
        self.backend = backend
    ...
    def naive_batching_generate(self, prompt, gen_length=128, block_length=128):
        ...
        for block_id, (block_loc, block) in enumerate(it):
            ...
            # In DP mode, sync early_stop across all ranks to prevent AllToAll deadlock
            if self.early_stop:
                local_all_done = torch.all(decode_compl)
                try:
                    from vllm.config import get_current_vllm_config
                    _cfg = get_current_vllm_config()
                    if _cfg is not None and _cfg.parallel_config.data_parallel_size > 1:
                        import torch.distributed as _dist
                        from vllm.distributed import get_dp_group
                        _done_tensor = local_all_done.int().clone()
                        _dist.all_reduce(_done_tensor, op=_dist.ReduceOp.MIN,
                                         group=get_dp_group().device_group)
                        if _done_tensor.item():
                            break
                    else:
                        if local_all_done:
                            break
