    ...
    def update(self, key_states, val_states, layer_idx, replace_position=None, backend='vllm'):
        ...
        if backend ==  'vllm':
            ...
            if replace_position is not None:
                if self.use_inplace_update and isinstance(self._data, torch.Tensor):
                    # In-place write: only copy block portion, return view
                    self._data[layer_idx, 0, :, :, replace_position[0]:replace_position[1], :] = key_states
                    self._data[layer_idx, 1, :, :, replace_position[0]:replace_position[1], :] = val_states
                    keys = self._data[layer_idx, 0]
                    values = self._data[layer_idx, 1]
                else:
